...
.
you can add more station to process by adding your stationconfiguration inside resources/station_list.csv, but it need to be in seedlink score of iris or your own seedlink api, you can change your seedlink endpoint inside .env.
note : you need to specify the sampling rate, the system expect fixed amount of sampling rate for each station. also the station need to be 3 channel sensor (eg. NZE, Z12. specify the channel orientation inside the resources/station_list.csv), because that what EqTransformer trained on <br>
...
you can run this on your own computer in any os for free (i think)
